...
public:
	...
	inline glm::mat4 projectionMatrix() const { return glm::perspective(fov, (float)viewport.x / (float)viewport.y, near, far); };
...
void XsDefaultCamera(XsCamera& cam) {
	...
	cam.viewMatrix() = glm::lookAt(position, position + rott, up) * tmx;
	glm::mat4 _ftransform = pmx * cam.viewMatrix();
...
void XsFPSCamera(XsCamera& cam, const vex2f& _mouse_pos, const float& _sensivity) {
	...
	glm::vec3 up = glm::normalize(glm::cross(right, rott));
	cam.viewMatrix() = glm::lookAt(position, position + rott, up) * tmx;
	glm::mat4 xs_fullTransform_ = pmx * cam.viewMatrix();
	cam.rot << t_rot;
