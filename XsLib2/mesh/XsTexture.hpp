...
public:
    ...
    auto& createCheckerA(const u32& x = 2u, const u32& y = 2u, const vex4f& first_color = 1, const vex4f& second_color = { 0,0,0,1 }) {
        std::vector<vex4ub> _data(x * y);
        bool s = false, l = false;
        u32 j = 0u;
        const vex4ub& fc = first_color * 255;
        const vex4ub& sc = second_color * 255;
        for (auto& i : _data) {
            i = s ? fc : sc;
            s = !s;
            j++;
